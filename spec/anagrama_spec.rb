...
describe('ANAGRAM#checker') do
  ...
  it("6) returns the anagram/antigram/status of each word entered as an argument compared to the primary word.") do
    ...
    expect(test.anagram("pe'ars~mu@nch~spaz s~spear")).to(eq("From your primary word of parse: pears is an anagram, spear is an anagram, munch is an antigram, spazs is boring."))
  end
